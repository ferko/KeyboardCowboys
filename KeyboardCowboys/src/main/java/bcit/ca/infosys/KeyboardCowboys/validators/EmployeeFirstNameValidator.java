 ...
 * @author Nem
 * 
 *         EmployeNameValidator validates that the Employee first name is between 3 to
 *         20 characters and can only contain alphabetic characters and space
 */
@FacesValidator("employeeFirstNameValidator")
public class EmployeeFirstNameValidator implements Validator {
    private Pattern pattern;
    private Matcher matcher;
    private static final String VALID_NAME_REGEX = "^[a-zA-z '-]{3,20}+$";

    public EmployeeFirstNameValidator() {
    ...
    public void validate(FacesContext context, UIComponent component,
        ...
        if (!matcher.matches()) {
            FacesMessage msg = new FacesMessage(
                    "Employee First Name must be between three to twenty characters long "
